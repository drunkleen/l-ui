...
const outDir = path.resolve(__dirname, '../hub/web/dist');
const BACKEND_TARGET = process.env.LUI_BACKEND_TARGET || 'http://localhost:2053';
...
function resolveDBPath() {
  const envFolder = process.env.LUI_DB_FOLDER;
  if (envFolder) {
    const abs = path.isAbsolute(envFolder)
      ? envFolder
      : path.resolve(__dirname, '..', envFolder);
    return path.join(abs, 'l-ui.db');
  }
  const repoSubDB = path.resolve(__dirname, '..', 'l-ui', 'l-ui.db');
  if (fs.existsSync(repoSubDB)) return repoSubDB;
  const repoDB = path.resolve(__dirname, '..', 'l-ui.db');
  if (fs.existsSync(repoDB)) return repoDB;
  return '/etc/l-ui/l-ui.db';
}
...
function injectBasePathPlugin() {
  return {
    ...
    transformIndexHtml(html) {
      ...
      const tag = `<script>window.L_UI_BASE_PATH="${escaped}";window.L_UI_CUR_VER="${version}";</script>`;
...
export default defineConfig({
  ...
  experimental: {
    renderBuiltUrl(filename, { hostType }) {
      if (hostType === 'js') {
        return {
          runtime: `((window.L_UI_BASE_PATH||'/')+${JSON.stringify(filename)})`,
        };
      }
      return undefined;
    },
