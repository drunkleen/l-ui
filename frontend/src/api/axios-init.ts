...
import { getApiPrefix } from '@/utils/runtime';

const SAFE_METHODS = new Set(['GET', 'HEAD', 'OPTIONS', 'TRACE']);
const CSRF_TOKEN_PATH = '/csrf-token';
const PANEL_API_PREFIX = '/panel/api';
...
function rewriteApiUrl(url?: string): string | undefined {
  if (!url) return url;
  const prefix = getApiPrefix();
  if (prefix !== PANEL_API_PREFIX && (url === PANEL_API_PREFIX || url.startsWith(`${PANEL_API_PREFIX}/`))) {
    return prefix + url.slice(PANEL_API_PREFIX.length);
  }
  return url;
}
...
export function setupAxios(): void {
  ...
  let basePath: string | null | undefined = window.L_UI_BASE_PATH;
  if (!basePath) {
