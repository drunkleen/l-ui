...
import { getPanelMode } from '@/utils/runtime';
import './AppSidebar.css';

const SIDEBAR_COLLAPSED_KEY = 'isSidebarCollapsed';
const DONATE_URL = 'https://drunkleen.com/support';
const REPO_URL = 'https://github.com/drunkleen/l-ui';
...
export default function AppSidebar() {
  ...
  const isHubMode = getPanelMode() === 'hub';
  ...
  const panelVersion = window.L_UI_CUR_VER || '';
  const { nodes } = useNodesQuery();
  const hasNodes = useMemo(() => Array.isArray(nodes) && nodes.length > 0, [nodes]);
  ...
  const tabs = useMemo<{ key: string; icon: IconName; title: string }[]>(() => {
    ...
    if (!isHubMode) {
      items.push({ key: '/xray', icon: 'tool', title: t('menu.xray') });
    }
    items.push(
  ...
  const xrayActive = !isHubMode && pathname === '/xray';
  ...
  const openLink = useCallback(async (key: string) => {
    if (key === LOGOUT_KEY) {
      ...
      window.location.href = window.L_UI_BASE_PATH || '/';
      return;
    }
    navigate(key);
  }, [navigate]);
    ...
    <div className="ant-sidebar">
      ...
      >
        <div className={`sider-brand${collapsed ? ' sider-brand-collapsed' : ''}`}>
          <div className="brand-block">
            <span className="brand-text">L-UI</span>
          </div>
          {!collapsed && (
      ...
      >
        <div className="drawer-header">
          <div className="brand-block">
            <span className="drawer-brand">L-UI</span>
          </div>
