...
export const AllSettingSchema = z.object({
  ...
  nodeCpuThreshold: z.number().int().min(0).max(100).optional(),
  nodeMemThreshold: z.number().int().min(0).max(100).optional(),
  nodeDiskThreshold: z.number().int().min(0).max(100).optional(),
  nodeDownThreshold: z.number().int().min(1).max(360).optional(),
}).loose();
