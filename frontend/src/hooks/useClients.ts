...
async function fetchClientPage(params: ClientQueryParams): Promise<ClientPageResponse> {
  ...
  const pagedMsg = await HttpUtil.get(`/panel/api/clients/list/paged?${qs}`, undefined, { silent: true });
  if (pagedMsg?.success && pagedMsg.obj) {
    const validated = parseMsg(pagedMsg, ClientPageResponseSchema, 'clients/list/paged');
    if (validated.obj) return validated.obj;
  }

  const fallbackMsg = await HttpUtil.get('/panel/api/clients/list', undefined, { silent: true });
  if (!fallbackMsg?.success || !fallbackMsg.obj) {
    throw new Error(pagedMsg?.msg || fallbackMsg?.msg || 'Failed to fetch clients');
  }
  const fallbackValidated = parseMsg(fallbackMsg, ClientRecordListSchema, 'clients/list');
  const items = fallbackValidated.obj || [];
  return {
    items,
    total: items.length,
    filtered: items.length,
    page: params.page > 0 ? params.page : 1,
    pageSize: params.pageSize > 0 ? params.pageSize : DEFAULT_QUERY.pageSize,
...
export function useClients() {
  ...
  const moveMut = useMutation({
    mutationFn: async (payload: { email: string; sourceNodeId: number; targetNodeId: number; targetInboundId: number }): Promise<Msg<ClientMoveResult>> => {
      const raw = await HttpUtil.post(`/panel/api/clients/${encodeURIComponent(payload.email)}/move`, {
        sourceNodeId: payload.sourceNodeId,
        targetNodeId: payload.targetNodeId,
        targetInboundId: payload.targetInboundId,
      }, JSON_HEADERS);
      return parseMsg(raw, ClientMoveResultSchema, 'clients/move');
    },
    onSuccess: (msg) => { if (msg?.success) invalidateAll(); },
  });

  const bulkMoveMut = useMutation({
    mutationFn: async (payload: { emails: string[]; sourceNodeId: number; targetNodeId: number; targetInboundId: number }): Promise<Msg<ClientMoveResult>> => {
      const raw = await HttpUtil.post('/panel/api/clients/bulkMove', payload, JSON_HEADERS);
      return parseMsg(raw, ClientMoveResultSchema, 'clients/bulkMove');
    },
    onSuccess: (msg) => { if (msg?.success) invalidateAll(); },
  });
  ...
  const move = useCallback((email: string, sourceNodeId: number, targetNodeId: number, targetInboundId: number) => {
    if (!email) return Promise.resolve(null as unknown as Msg<ClientMoveResult>);
    return moveMut.mutateAsync({ email, sourceNodeId, targetNodeId, targetInboundId });
  }, [moveMut]);
  const bulkMove = useCallback((emails: string[], sourceNodeId: number, targetNodeId: number, targetInboundId: number) => {
    if (!Array.isArray(emails) || emails.length === 0) return Promise.resolve(null as unknown as Msg<ClientMoveResult>);
    return bulkMoveMut.mutateAsync({ emails, sourceNodeId, targetNodeId, targetInboundId });
  }, [bulkMoveMut]);
