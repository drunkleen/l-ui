...
export function useInboundColumns({
  ...
  return useMemo(() => {
    ...
    cols.push({
      title: t('pages.inbounds.owner'),
      key: 'owner',
      align: 'center',
      width: 70,
      render: (_, record) => {
        if (record.userId == null) return <Tag color="default">{t('pages.inbounds.ownerDefault')}</Tag>;
        const user = usersById.get(record.userId);
        if (!user) return <Tag color="orange">user #{record.userId}</Tag>;
        return <Tag color="geekblue">{user.username}</Tag>;
      },
    });
    ...
    return cols;
  }, [t, hasAnyRemark, hasActiveNode, nodesById, usersById, clientCount, subEnable, expireDiff, trafficDiff, datepicker, onRowAction, onSwitchEnable]);
}
