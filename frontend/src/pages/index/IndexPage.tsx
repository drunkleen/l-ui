...
import { usePanelContext } from '@/api/queries/usePanelContext';
...
export default function IndexPage() {
  ...
  const isHubMode = context.mode === 'hub';
  const { nodes, loading: nodesLoading, fetched: nodesFetched, fetchError: nodesFetchError, refetch: refetchNodes } = useNodesQuery({ enabled: isHubMode, refetchIntervalMs: isHubMode ? 10_000 : false });
  ...
  const basePath = window.L_UI_BASE_PATH || '';
  ...
  const displayVersion = useMemo(
    () => panelUpdateInfo.currentVersion || context.version || window.L_UI_CUR_VER || '?',
    [panelUpdateInfo.currentVersion, context.version],
  );

  const dashboardLoading = isHubMode ? (nodesLoading || !fetched || !nodesFetched) : !fetched;
  const dashboardReady = isHubMode ? (fetched && nodesFetched) : fetched;
  const dashboardError = isHubMode ? (nodesFetchError || fetchError) : fetchError;
  const refreshDashboard = useCallback(async () => {
    if (isHubMode) {
      await Promise.all([refresh(), refetchNodes()]);
      return;
    }
    await refresh();
  }, [isHubMode, refetchNodes, refresh]);
  ...
  function openPanelVersion() {
    if (panelUpdateInfo.updateAvailable) {
      ...
      window.open('https://github.com/drunkleen/l-ui/releases', '_blank', 'noopener,noreferrer');
    }
  }
            ...
            >
              {!dashboardReady ? (
                ...
                <Result
                  ...
                  extra={<Button type="primary" onClick={refreshDashboard}>{t('refresh')}</Button>}
                />
              ) : isHubMode ? (
                <HubOverview context={context} status={status} nodes={nodes} />
              ) : (
