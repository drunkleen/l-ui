...
const ClientMoveModal = lazy(() => import('./ClientMoveModal'));
...
export default function ClientsPage() {
  ...
  const {
    ...
    allGroups,
    setQuery,
    inbounds, onlines, loading, fetchError, subSettings,
    ipLimitEnable, tgBotEnable, expireDiff, trafficDiff, pageSize,
    create, update, remove, bulkDelete, bulkAdjust, bulkAddToGroup, bulkRemoveFromGroup, attach, bulkAttach, detach, bulkDetach, move, bulkMove,
  ...
  const [moveEmails, setMoveEmails] = useState<string[]>([]);
  const [moveDefaultSourceNodeId, setMoveDefaultSourceNodeId] = useState<number | undefined>(undefined);
  ...
  const showFatalFetchError = !hasListData && !!fetchError;
  const showFetchWarning = hasListData && !!fetchError;
  ...
  const nodesById = useMemo(() => {
    const out: Record<number, NodeRecord> = {};
  ...
  function inboundLabel(id: number) {
    ...
    const node = ib?.nodeId ? nodesById[ib.nodeId] : undefined;
    const base = ib?.remark?.trim() || ib?.tag || '';
    const nodePart = node?.name?.trim() || node?.address?.trim() || '';
    return nodePart ? `${base} @ ${nodePart}` : base;
  }

  function clientDefaultSourceNodeId(row: ClientRecord | null | undefined): number | undefined {
    if (!row?.inboundIds?.length) return undefined;
    for (const inboundId of row.inboundIds) {
      const nodeId = inboundsById[inboundId]?.nodeId;
      if (typeof nodeId === 'number' && nodeId > 0) return nodeId;
    }
    return undefined;
  }

  function openMove(emails: string[], sourceNodeId?: number) {
    if (emails.length === 0) return;
    setMoveEmails(emails);
    setMoveDefaultSourceNodeId(sourceNodeId);
    setMoveOpen(true);
  }
  ...
  function onMove(row: ClientRecord) {
    openMove([row.email], clientDefaultSourceNodeId(row));
  }
  ...
  function onBulkMove() {
    const emails = [...selectedRowKeys];
    if (emails.length === 0) return;
    const first = clients.find((row) => emails.includes(row.email));
    openMove(emails, clientDefaultSourceNodeId(first));
  }
  ...
  async function submitMove(payload: {
    ...
    if (payload.emails.length === 1) {
      return move(payload.emails[0], payload.sourceNodeId, payload.targetNodeId, payload.targetInboundId);
    }
    return bulkMove(payload.emails, payload.sourceNodeId, payload.targetNodeId, payload.targetInboundId);
  }
      ...
      <Layout className={pageClass}>
        ...
        <Layout className="content-shell">
          <Layout.Content id="content-layout" className="content-area">
            <Spin spinning={!hasListData && !showFatalFetchError} delay={200} description={t('loading')} size="large">
              {showFatalFetchError ? (
                <Result
                  status="error"
                ...
                <>
                  {showFetchWarning ? (
                    <Alert
                      className="clients-load-alert"
                      type="warning"
                      title={t('somethingWentWrong')}
                      description={fetchError}
                      action={<Button type="primary" loading={loading} onClick={refresh}>{t('refresh')}</Button>}
                      showIcon
                    />
                  ) : null}
                        ...
                        <div className="card-toolbar">
                          {selectedRowKeys.length === 0 ? (
                            ...
                            <>
                              ...
                              <Button icon={<SwapOutlined />} onClick={onBulkMove}>
                                {!isMobile && t('pages.clients.move', { defaultValue: 'Move' })}
                              </Button>
                              ...
                              items: selectedRowKeys.length > 0
                                ? [
                                    ...
                                    },
                                    {
                                      key: 'move',
                                      icon: <SwapOutlined />,
                                      label: t('pages.clients.move', { defaultValue: 'Move' }),
                                      onClick: () => openMove([...selectedRowKeys]),
                                    },
                                  ]
                                : [
                                    {
                                      ...
                                      <Dropdown
                                        ...
                                        menu={{
                                          items: [
                                            ...
                                            {
                                              key: 'move',
                                              label: <><SwapOutlined /> {t('pages.clients.move', { defaultValue: 'Move' })}</>,
                                              onClick: () => onMove(row),
                                            },
                                            {
                                              key: 'edit',
        ...
        <LazyMount when={moveOpen}>
          <ClientMoveModal
            open={moveOpen}
            count={moveEmails.length}
            emails={moveEmails}
            nodes={nodes}
            inbounds={inbounds}
            defaultSourceNodeId={moveDefaultSourceNodeId}
            onOpenChange={setMoveOpen}
            onSubmit={async (payload) => {
              const msg = await submitMove(payload);
              if (msg?.success) {
                setSelectedRowKeys([]);
                return msg.obj ?? { moved: [], skipped: [], errors: [] };
              }
              return null;
            }}
          />
