      ...
      },
      {
        ...
        children: (
          <>
            ...
            <SettingListItem paddings="small" title={t('pages.settings.nodeCpuThreshold') || 'Node CPU threshold'} description={t('pages.settings.nodeCpuThresholdDesc') || 'CPU % threshold for node alerts'}>
              <InputNumber value={allSetting.nodeCpuThreshold} min={0} max={100} style={{ width: '100%' }}
                onChange={(v) => updateSetting({ nodeCpuThreshold: Number(v) || 0 })} />
            </SettingListItem>
            <SettingListItem paddings="small" title={t('pages.settings.nodeMemThreshold') || 'Node Memory threshold'} description={t('pages.settings.nodeMemThresholdDesc') || 'Memory % threshold for node alerts'}>
              <InputNumber value={allSetting.nodeMemThreshold} min={0} max={100} style={{ width: '100%' }}
                onChange={(v) => updateSetting({ nodeMemThreshold: Number(v) || 0 })} />
            </SettingListItem>
            <SettingListItem paddings="small" title={t('pages.settings.nodeDiskThreshold') || 'Node Disk threshold'} description={t('pages.settings.nodeDiskThresholdDesc') || 'Disk % threshold for node alerts'}>
              <InputNumber value={allSetting.nodeDiskThreshold} min={0} max={100} style={{ width: '100%' }}
                onChange={(v) => updateSetting({ nodeDiskThreshold: Number(v) || 0 })} />
            </SettingListItem>
            <SettingListItem paddings="small" title={t('pages.settings.nodeDownThreshold') || 'Node down threshold'} description={t('pages.settings.nodeDownThresholdDesc') || 'Consecutive failed checks before node-down alert'}>
              <InputNumber value={allSetting.nodeDownThreshold} min={1} max={360} style={{ width: '100%' }}
                onChange={(v) => updateSetting({ nodeDownThreshold: Number(v) || 3 })} />
            </SettingListItem>
          </>
        ),
      },
