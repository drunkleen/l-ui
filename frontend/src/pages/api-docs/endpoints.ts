...
export const sections: readonly Section[] = [
...

  {
    ...
    endpoints: [
      ...
      },
      {
        ...
        summary: 'Create a new inbound. Send the full inbound payload (protocol, port, settings, streamSettings, sniffing, remark, userId, expiryTime, total, enable). settings, streamSettings, and sniffing may be sent as nested JSON objects (preferred) or as JSON-encoded strings (legacy).',
        body:
      ...
      },
      {
        ...
        summary: 'Replace an inbound’s configuration. Body shape mirrors /add, including userId for owner reassignment. Heavy on inbounds with thousands of clients — prefer /setEnable for enable-only flips.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Inbound ID.' },
        ],
      },
...

  {
    ...
    title: 'Server',
    description:
      'System status, hub-safe context, log retrieval, certificate generators, Xray binary management, and backup/restore. All under /panel/api/server.',
    endpoints: [
      {
        method: 'GET',
      ...
      {
        method: 'GET',
        path: '/panel/api/server/context',
        summary: 'Panel runtime context for mode-aware frontend rendering. Returns whether the panel is in hub or agent mode, the configured DB backend, API prefix, and whether local Xray controls are enabled.',
      ...
      {
        method: 'GET',
        path: '/panel/api/server/health',
        summary: 'Lightweight liveness check for the current node backend.',
      },
      {
        method: 'GET',
      ...
      {
        method: 'POST',
        path: '/panel/api/server/cleanup',
        summary: 'Stop node services and remove installed node files from the VPS.',
      },
      {
        method: 'POST',
        path: '/panel/api/server/reinstall',
        summary: 'Upload a node bundle to the agent and reinstall it on the VPS.',
      },
      {
        method: 'GET',
        path: '/panel/api/server/ufw',
        summary: 'List the current UFW rules on the node-agent host.',
      },
      {
        method: 'POST',
        path: '/panel/api/server/ufw/allow',
        summary: 'Allow a TCP or UDP port on the node-agent host.',
        body: '{\n  "port": 2053,\n  "protocol": "tcp"\n}',
      },
      {
        method: 'POST',
        path: '/panel/api/server/ufw/deny',
        summary: 'Deny a TCP or UDP port on the node-agent host.',
        body: '{\n  "port": 2053,\n  "protocol": "udp"\n}',
      },
      {
        method: 'POST',
        path: '/panel/api/server/rotateToken',
        summary: 'Rotate the node bootstrap/API token on the agent.',
      },
      {
        method: 'POST',
...

  {
    ...
    endpoints: [
      ...
      {
        method: 'POST',
        path: '/panel/api/clients/:email/move',
        summary: 'Move a client from a source node to a target node/inbound. The client record and fields stay intact; only the attachments are re-homed.',
        params: [
          { name: 'email', in: 'path', type: 'string', desc: 'Client email (unique identifier).' },
        ],
        body: '{\n  "sourceNodeId": 1,\n  "targetNodeId": 2,\n  "targetInboundId": 18\n}',
      ...
      {
        method: 'POST',
        path: '/panel/api/clients/bulkMove',
        summary: 'Move many clients from one node to another node/inbound. The server reports moved, skipped, and error rows per email so the hub can show partial success clearly.',
        body: '{\n  "emails": ["alice", "bob"],\n  "sourceNodeId": 1,\n  "targetNodeId": 2,\n  "targetInboundId": 18\n}',
        response: '{\n  "success": true,\n  "obj": {\n    "moved": ["alice@example.com"],\n    "skipped": [],\n    "errors": []\n  }\n}',
      },
      {
    ...
    endpoints: [
      {
        ...
        summary: 'List every configured node with its name, connection details, health, and last heartbeat patch.',
      ...
      },
      {
        ...
        summary: 'Register a new remote node. Provide its name, URL, apiToken, and optional allowPrivateAddress flag.',
        body:
      ...
      },
      {
        ...
        summary: 'SSH into a new host, install the l-ui agent, and optionally publish it behind Caddy with HTTPS. The node name comes from `name`; `agentPort` is optional and defaults to a random port above 2000 when omitted. The panel generates the API token and returns a tracked bootstrap job for live progress display.',
        body:
          '{\n  "name": "de-fra-1",\n  "address": "203.0.113.10",\n  "sshUser": "root",\n  "sshPassword": "secret",\n  "useTLS": true,\n  "domain": "node.example.com",\n  "acmeEmail": "admin@example.com",\n  "sshPort": 22\n}',
        response:
          '{\n  "success": true,\n  "obj": {\n    "id": "bootstrap-xyz",\n    "state": "running",\n    "step": "download-install",\n    "steps": [\n      { "name": "detect-arch", "ok": true, "output": "x86_64" },\n      { "name": "release-tag", "ok": true, "output": "v0.0.1" }\n    ]\n  }\n}',
      },
      {
        method: 'GET',
        path: '/panel/api/nodes/bootstrap/:id',
        summary: 'Poll the current state of a bootstrap job created by /bootstrap. Use this for progress UI until the job reaches done or failed.',
        params: [
          { name: 'id', in: 'path', type: 'string', desc: 'Bootstrap job ID.' },
        ],
        response:
          '{\n  "success": true,\n  "obj": {\n    "id": "bootstrap-xyz",\n    "state": "done",\n    "node": { "id": 1, "name": "de-fra-1" },\n    "steps": [\n      { "name": "verify-agent", "ok": true, "output": "{...}" }\n    ]\n  }\n}',
      },
      {
        method: 'POST',
        path: '/panel/api/nodes/update/:id',
        summary: 'Replace a node\u2019s connection details. Same body shape as /add; `name` is the user-facing label.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
        ],
        body: '{\n  "name": "de-fra-1",\n  "scheme": "https",\n  "address": "node1.example.com",\n  "port": 2053,\n  "basePath": "/",\n  "apiToken": "abcdef...",\n  "enable": true,\n  "allowPrivateAddress": false\n}',
      ...
      {
        method: 'POST',
        path: '/panel/api/nodes/cert/:id/generate',
        summary: 'Generate a TLS certificate for the node signed by the hub CA and push it to the node.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
        ],
        response: '{\n  "success": true\n}',
      ...
      {
        method: 'GET',
        path: '/panel/api/nodes/cert/:id/status',
        summary: 'Get the node\'s current TLS certificate info (subject, issuer, serial, validity, fingerprint).',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
        ],
      ...
      {
        method: 'POST',
        path: '/panel/api/nodes/cert/renew',
        summary: 'Check all nodes and renew TLS certificates that expire within 30 days.',
        response: '{\n  "success": true,\n  "obj": [\n    { "id": 1, "name": "de-1" },\n    { "id": 2, "name": "fr-1", "error": "push failed" }\n  ]\n}',
      },
      {
        method: 'POST',
      ...
      {
        method: 'POST',
        path: '/panel/api/nodes/reinstall/:id',
        summary: 'Reinstall the node bundle on a single node using the current hub artifacts, preserving the node record and API token.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
        ],
      },
      {
        method: 'POST',
        path: '/panel/api/nodes/rotateCredentials/:id',
        summary: 'Rotate a node API token and reconfigure the remote node to use the new credential.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
        ],
      },
      {
        method: 'POST',
        path: '/panel/api/nodes/reconcile/:id',
        summary: 'Reprobe a node and attempt a recovery restart if the node is unhealthy.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
        ],
      },
      {
        method: 'GET',
        path: '/panel/api/nodes/ufw/:id',
        summary: 'List the current UFW rules for a node.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
        ],
      },
      {
        method: 'POST',
        path: '/panel/api/nodes/ufw/:id/allow',
        summary: 'Allow a TCP or UDP port on a node.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
        ],
        body: '{\n  "port": 2053,\n  "protocol": "tcp"\n}',
      ...
      {
        method: 'POST',
        path: '/panel/api/nodes/ufw/:id/deny',
        summary: 'Deny a TCP or UDP port on a node.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
        ],
        body: '{\n  "port": 2053,\n  "protocol": "udp"\n}',
      ...
      {
        method: 'POST',
        path: '/panel/api/nodes/ufw/:id/delete',
        summary: 'Delete a UFW rule by its rule number on a node.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
        ],
        body: '{\n  "rule_number": "1"\n}',
      ...
      {
        method: 'POST',
        path: '/panel/api/nodes/ufw/:id/enable',
        summary: 'Enable UFW on a node.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
        ],
      },
      {
        method: 'POST',
        path: '/panel/api/nodes/ufw/:id/disable',
        summary: 'Disable UFW on a node.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
        ],
      },
      {
        method: 'GET',
        path: '/panel/api/nodes/groups',
        summary: 'List distinct node group labels.',
        response: '{\n  "success": true,\n  "obj": ["eu", "asia"]\n}',
      },
      {
        method: 'POST',
        path: '/panel/api/nodes/:id/setGroup',
        summary: 'Set the group label on a single node.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
        ],
        body: '{\n  "group": "eu"\n}',
      ...
      {
        method: 'GET',
        path: '/panel/api/nodes/portGroup/list',
        summary: 'List all port groups.',
        response: '{\n  "success": true,\n  "obj": [\n    {\n      "id": 1,\n      "name": "Web",\n      "ports": "[{\\"port\\":80,\\"protocol\\":\\"tcp\\",\\"comment\\":\\"HTTP\\"},{\\"port\\":443,\\"protocol\\":\\"tcp\\",\\"comment\\":\\"HTTPS\\"}]"\n    }\n  ]\n}',
      },
      {
        method: 'POST',
        path: '/panel/api/nodes/portGroup/add',
        summary: 'Create a new port group.',
        body: '{\n  "name": "Web",\n  "ports": [{"port": 80, "protocol": "tcp", "comment": "HTTP"}, {"port": 443, "protocol": "tcp", "comment": "HTTPS"}]\n}',
      },
      {
        method: 'POST',
        path: '/panel/api/nodes/portGroup/update/:id',
        summary: 'Update an existing port group.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Port group ID.' },
        ],
        body: '{\n  "name": "Web",\n  "ports": [{"port": 80, "protocol": "tcp", "comment": "HTTP"}]\n}',
      },
      {
        method: 'POST',
        path: '/panel/api/nodes/portGroup/del/:id',
        summary: 'Delete a port group.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Port group ID.' },
        ],
      },
      {
        method: 'POST',
        path: '/panel/api/nodes/portGroup/push/:id/:group',
        summary: 'Push all ports from a port group to nodes with the given group label. Opens UFW allow rules on each matching node.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Port group ID.' },
          { name: 'group', in: 'path', type: 'string', desc: 'Node group label to target.' },
        ],
        response: '{\n  "success": true,\n  "msg": "Update successful"\n}',
      },
      {
        method: 'POST',
        path: '/panel/api/nodes/pushConfig/:id',
        summary: 'Push the hub\'s Xray config and client list to a remote node. The agent applies the config and reports back the new config version.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
        ],
        response: '{\n  "success": true,\n  "msg": "Update successful"\n}',
      ...
      {
        method: 'POST',
        path: '/panel/api/nodes/updateXray/:id/:version',
        summary: 'Install a specific Xray version on a remote node.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
          { name: 'version', in: 'path', type: 'string', desc: 'Xray version or latest.' },
        ],
      },
      {
        method: 'GET',
      ...
      {
        method: 'GET',
        path: '/panel/api/nodes/logs/:id',
        summary: 'Fetch recent agent logs from a node. Supports lines, filter, showDirect, showBlocked, showProxy query params — same as the local /server/xraylogs/:count endpoint.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
          { name: 'lines', in: 'query', type: 'number', desc: 'Number of trailing log lines. Defaults to 50.' },
          { name: 'filter', in: 'query', type: 'string', desc: 'Keyword filter — only lines containing this string.' },
          { name: 'showDirect', in: 'query', type: 'string', desc: '"true" to include direct (freedom) traffic lines.' },
          { name: 'showBlocked', in: 'query', type: 'string', desc: '"true" to include blocked (blackhole) traffic lines.' },
          { name: 'showProxy', in: 'query', type: 'string', desc: '"true" to include proxy traffic lines.' },
        ],
      },
      {
        method: 'POST',
        path: '/panel/api/nodes/restart/:id',
        summary: 'Restart the l-ui agent process on a remote node. The node briefly goes offline and reconnects with its stored credentials.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
        ],
      },
      {
        method: 'POST',
        path: '/panel/api/nodes/xray/restart/:id',
        summary: 'Restart the Xray process on a remote node without restarting the agent itself. Useful after config changes that need a clean Xray reload.',
        params: [
          { name: 'id', in: 'path', type: 'number', desc: 'Node ID.' },
        ],
      },
...

  {
    ...
    endpoints: [
      ...
      {
        method: 'GET',
        path: '/panel/setting/users',
        summary: 'List panel users so inbounds can be assigned to a specific hub account.',
        response: '{\n  "success": true,\n  "obj": [\n    {\n      "id": 1,\n      "username": "admin"\n    }\n  ]\n}',
      },
      {
        method: 'POST',
