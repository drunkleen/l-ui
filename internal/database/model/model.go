...
import (
	...
	"github.com/drunkleen/l-ui/internal/util/json_util"
...
// User represents a user account in the l-ui panel.
type User struct {
...
type Inbound struct {
	...
	UserId               int                  `json:"userId,omitempty" form:"userId"`                                                                                                                               // Associated user ID
...
// Setting stores key-value configuration settings for the l-ui panel.
type Setting struct {
	Id    int    `json:"id" form:"id" gorm:"primaryKey;autoIncrement"`
...
// Node represents a remote l-ui panel registered with the central panel.
...
type Node struct {
	...
	BundleSHA256        string `json:"bundleSha256" gorm:"column:bundle_sha256;default:''"`
	...
	DiskCurrent   uint64  `json:"diskCurrent" gorm:"column:disk_current;default:0"`
	DiskTotal     uint64  `json:"diskTotal" gorm:"column:disk_total;default:0"`
	NetUp         uint64  `json:"netUp" gorm:"column:net_up;default:0"`
	NetDown       uint64  `json:"netDown" gorm:"column:net_down;default:0"`
	...
	ConfigVersion int     `json:"configVersion" gorm:"default:0"`
	Group         string  `json:"group" gorm:"column:group_name;default:''"`
	CertSerial    string  `json:"certSerial" gorm:"column:cert_serial;default:''"`
	CertExpiry    int64   `json:"certExpiry" gorm:"column:cert_expiry;default:0"`
	...
	DriftReasons  []string `json:"driftReasons" gorm:"-"`
...
// PortGroup defines a named collection of port+protocol entries that can be
// pushed to one or more nodes (by group label) to automatically open UFW ports.
type PortGroup struct {
	Id        int    `json:"id" gorm:"primaryKey;autoIncrement"`
	Name      string `json:"name" gorm:"uniqueIndex;not null"`
	Ports     string `json:"ports"` // JSON array of {port, protocol, comment}
	CreatedAt int64  `json:"createdAt" gorm:"autoCreateTime:milli"`
	UpdatedAt int64  `json:"updatedAt" gorm:"autoUpdateTime:milli"`
}

// NodeBundle tracks prepared node installation bundles built by the hub.
type NodeBundle struct {
	Id        int    `json:"id" gorm:"primaryKey;autoIncrement"`
	Name      string `json:"name" gorm:"uniqueIndex;not null"`
	Version   string `json:"version" gorm:"index;not null;default:''"`
	Arch      string `json:"arch" gorm:"index;not null"`
	SHA256    string `json:"sha256" gorm:"uniqueIndex;not null"`
	Path      string `json:"path" gorm:"not null"`
	Manifest  string `json:"manifest" gorm:"column:manifest_path"`
	Size      int64  `json:"size"`
	BuiltAt   int64  `json:"builtAt" gorm:"column:built_at"`
	CreatedAt int64  `json:"createdAt" gorm:"autoCreateTime:milli"`
	UpdatedAt int64  `json:"updatedAt" gorm:"autoUpdateTime:milli"`
}

// NodeRegistrationToken is a one-time token that lets an agent self-register
// with the hub without SSH access. The panel operator generates these tokens
// and passes them to the agent via environment variable (LUI_REGISTRATION_TOKEN).
type NodeRegistrationToken struct {
	...
	NodeAddress      string `json:"nodeAddress" gorm:"size:128"`
	ConsumedByNodeID int    `json:"consumedByNodeId" gorm:"column:consumed_by_node_id;default:0"`
	ConsumedAt       int64  `json:"consumedAt" gorm:"column:consumed_at;default:0"`
