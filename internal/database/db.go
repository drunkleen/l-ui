...
// for the l-ui panel using GORM with SQLite or PostgreSQL.
package database
...
func initModels() error {
	...
	if err := dropLegacyNodeColumns(); err != nil {
		return err
	}
...
func dropLegacyNodeColumns() error {
	if db == nil || !db.Migrator().HasColumn(&model.Node{}, "remark") {
		return nil
	}
	if err := db.Migrator().DropColumn(&model.Node{}, "remark"); err != nil {
		log.Printf("Error dropping legacy node.remark column: %v", err)
		return err
	}
	log.Printf("Dropped legacy node.remark column")
	return nil
}
...
// When LUI_DB_TYPE=postgres, dbPath is ignored and LUI_DB_DSN is used instead.
func InitDB(dbPath string) error {
	...
	case "postgres":
		dsn := config.GetDBDSN()
		if dsn == "" {
			return errors.New("LUI_DB_TYPE=postgres but LUI_DB_DSN is empty")
		}
	...
	case "mysql":
		dsn := config.GetDBDSN()
		if dsn == "" {
			return errors.New("LUI_DB_TYPE=mysql but LUI_DB_DSN is empty")
		}
	...
	switch config.GetDBKind() {
	case "postgres":
		maxOpen = envInt("LUI_DB_MAX_OPEN_CONNS", 25)
		maxIdle = envInt("LUI_DB_MAX_IDLE_CONNS", 25)
	case "mysql":
		maxOpen = envInt("LUI_DB_MAX_OPEN_CONNS", 25)
		maxIdle = envInt("LUI_DB_MAX_IDLE_CONNS", 25)
	default:
		maxOpen = envInt("LUI_DB_MAX_OPEN_CONNS", 8)
		maxIdle = envInt("LUI_DB_MAX_IDLE_CONNS", 4)
	}
