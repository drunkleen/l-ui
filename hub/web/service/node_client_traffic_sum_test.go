...
func TestNodeDelete_CleansNodeBaselines(t *testing.T) {
	db := initTrafficTestDB(t)
	nodeSvc := NodeService{}
	if err := db.Create(&model.Node{Id: 7, Name: "node-7", Address: "127.0.0.1", Port: 1, Scheme: "http", ApiToken: "tok", Enable: true}).Error; err != nil {
		t.Fatalf("seed node 7 record: %v", err)
	}
	...
	if err := nodeSvc.Delete(7, false); err != nil {
