// Package service provides business logic services for the l-ui web panel,
...
func (s *InboundService) GetInboundOptions(userId int) ([]InboundOption, error) {
	...
	err := db.Table("inbounds").
		Select("id, node_id, remark, tag, protocol, port, stream_settings").
...
// ufwNetworkProtocol returns the UFW network protocol ("tcp" or "udp") for an inbound.
// Defaults to "tcp" for most protocols; returns "udp" for hysteria, wireguard,
// or stream networks using kcp/quic.
func ufwNetworkProtocol(ib *model.Inbound) string {
	if ib.Protocol == "hysteria" || ib.Protocol == "wireguard" {
		return "udp"
	}
	var ss struct {
		Network string `json:"network"`
	...
	if ib.StreamSettings != "" {
		if err := json.Unmarshal([]byte(ib.StreamSettings), &ss); err == nil {
			if ss.Network == "kcp" || ss.Network == "quic" {
				return "udp"
			}
		}
	}
...
func (s *InboundService) autoOpenUfwForInbound(rt runtime.Runtime, ib *model.Inbound) {
	rem, ok := rt.(*runtime.Remote)
	if !ok {
		return
	}
	protocol := ufwNetworkProtocol(ib)
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	if err := rem.AllowUfwPort(ctx, ib.Port, protocol); err != nil {
		logger.Warning("autoOpenUfwForInbound: failed to allow port", ib.Port, "on node for inbound", ib.Tag, ":", err)
	} else {
		logger.Debug("autoOpenUfwForInbound: allowed port", ib.Port, "on node for inbound", ib.Tag)
	}
}

func (s *InboundService) autoCloseUfwForInbound(rt runtime.Runtime, ib *model.Inbound) {
	rem, ok := rt.(*runtime.Remote)
	if !ok {
		return
	}
	portStr := strconv.Itoa(ib.Port)
	// Check if other inbounds still use this port on the same node
	db := database.GetDB()
	var siblings int64
	db.Model(&model.Inbound{}).Where("port = ? AND node_id = ? AND id != ? AND enable = ?", ib.Port, ib.NodeID, ib.Id, true).Count(&siblings)
	if siblings > 0 {
		logger.Debug("autoCloseUfwForInbound: skipping close for port", portStr, "— other inbounds on node still use it")
		return
	}
	protocol := ufwNetworkProtocol(ib)
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	status, err := rem.ListUfwRules(ctx)
	if err != nil {
		logger.Warning("autoCloseUfwForInbound: failed to list UFW rules for inbound", ib.Tag, ":", err)
		return
	}
	for i := len(status.Rules) - 1; i >= 0; i-- {
		rule := status.Rules[i]
		if !strings.HasPrefix(rule.Port, portStr+"/") && rule.Port != portStr {
			continue
		}
		if rule.Action != "allow" {
			continue
		}
		if protocol != "" && rule.Protocol != "" && rule.Protocol != protocol {
			continue
		}
		if err := rem.DeleteUfwRule(ctx, strconv.Itoa(rule.Number)); err != nil {
			logger.Warning("autoCloseUfwForInbound: failed to delete UFW rule", rule.Number, "for inbound", ib.Tag, ":", err)
		} else {
			logger.Debug("autoCloseUfwForInbound: deleted UFW rule", rule.Number, "for inbound", ib.Tag)
		}
	}
}
...
func (s *InboundService) AddInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	if inbound.Enable {
		...
		if err1 := rt.AddInbound(context.Background(), inbound); err1 == nil {
			...
			// endpoint — they receive inbound changes via full config push.
			// Log the failure and let the user push config manually.
			needRestart = true
		}
		if inbound.NodeID != nil {
			s.autoOpenUfwForInbound(rt, inbound)
		}
	}
...
func (s *InboundService) DelInbound(id int) (bool, error) {
	...
	if loadErr == nil {
		...
		if shouldPushToRuntime {
			...
			if ib.NodeID != nil {
				s.autoCloseUfwForInbound(rt, &ib)
			}
		} else {
...
func (s *InboundService) UpdateInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	oldInbound.UserId = inbound.UserId
