...
func nodeIDMatches(nodeID *int, target int) bool {
	if target <= 0 {
		return nodeID == nil
	}
	return nodeID != nil && *nodeID == target
}

func filterInboundIDsByNode(inboundIDs []int, nodeByInbound map[int]*int, targetNodeID int) []int {
	if len(inboundIDs) == 0 {
		return nil
	}
	out := make([]int, 0, len(inboundIDs))
	for _, inboundID := range inboundIDs {
		if nodeIDMatches(nodeByInbound[inboundID], targetNodeID) {
...
func inboundNodeLookup(inbounds []*model.Inbound) map[int]*int {
	lookup := make(map[int]*int, len(inbounds))
	for _, ib := range inbounds {
		if ib == nil {
			continue
		}
		lookup[ib.Id] = ib.NodeID
	}
	return lookup
}
...
type ClientMoveResult struct {
	Moved   []string         `json:"moved,omitempty"`
	Skipped []BulkMoveReport `json:"skipped,omitempty"`
	Errors  []BulkMoveReport `json:"errors,omitempty"`
}

type BulkMoveReport struct {
...
func (s *ClientService) moveClient(inboundSvc *InboundService, email string, sourceNodeID, targetNodeID, targetInboundID int) (bool, error) {
	if email == "" {
		return false, common.NewError("client email is required")
	}
	if targetInboundID <= 0 {
		return false, common.NewError("target inbound is required")
	}
	if sourceNodeID == 0 && targetNodeID == 0 {
		return false, common.NewError("source node and target node are required")
	}
	targetInbound, err := inboundSvc.GetInbound(targetInboundID)
	if err != nil {
		return false, err
	}
	if !nodeIDMatches(targetInbound.NodeID, targetNodeID) {
		return false, common.NewError("target inbound does not belong to the selected node")
	}
	...
	nodeByInbound := inboundNodeLookup(inbounds)
	sourceIds := filterInboundIDsByNode(currentIds, nodeByInbound, sourceNodeID)
	if len(sourceIds) == 0 {
		...
		return false, common.NewError("client is not attached to the selected source node")
	}

	needRestart := false
	if nr, err := s.Detach(inboundSvc, rec.Id, sourceIds); err != nil {
		return needRestart, err
	} else if nr {
		needRestart = true
	}
	if nr, err := s.Attach(inboundSvc, rec.Id, []int{targetInboundID}); err != nil {
		if rbNr, rbErr := s.Attach(inboundSvc, rec.Id, sourceIds); rbErr == nil && rbNr {
			needRestart = true
		}
		return needRestart, err
	} else if nr {
...
// Move transfers a client from one node to another by detaching all inbounds on
// the source node and attaching the same client record to a single target inbound.
func (s *ClientService) Move(inboundSvc *InboundService, email string, sourceNodeID, targetNodeID, targetInboundID int) (bool, error) {
	return s.moveClient(inboundSvc, email, sourceNodeID, targetNodeID, targetInboundID)
}

func (s *ClientService) BulkMove(inboundSvc *InboundService, emails []string, sourceNodeID, targetNodeID, targetInboundID int) (*ClientMoveResult, bool, error) {
	result := &ClientMoveResult{}
	if len(emails) == 0 {
		return result, false, nil
	}
	...
	for _, email := range emails {
		...
		seen[key] = struct{}{}
		nr, err := s.moveClient(inboundSvc, email, sourceNodeID, targetNodeID, targetInboundID)
		if err != nil {
			if strings.Contains(err.Error(), "not attached to the selected source node") {
				result.Skipped = append(result.Skipped, BulkMoveReport{Email: email, Reason: err.Error()})
				continue
			}
			result.Errors = append(result.Errors, BulkMoveReport{Email: email, Reason: err.Error()})
			logger.Warningf("[BulkMove] %s: %v", email, err)
			continue
		}
		if nr {
			needRestart = true
		}
		result.Moved = append(result.Moved, email)
	}
	return result, needRestart, nil
}
	...
	for _, r := range removed {
		...
		if oldInbound.NodeID != nil && len(email) > 0 {
			...
			if err1 := rt.DeleteUser(context.Background(), oldInbound, email); err1 != nil {
				logger.Debug("DeleteUser on remote node failed (agent nodes don't expose this API):", err1)
			}
		}
	}
	...
	if rterr != nil {
		...
		for _, client := range clients {
			...
			if err1 := rt.AddClient(context.Background(), oldInbound, client); err1 != nil {
				logger.Debug("AddClient on", rt.Name(), "failed (agent nodes don't expose this API):", err1)
				// Remote nodes (agents) don't expose the /api/v1/inbounds/add
				// endpoint — client changes arrive via full config push.
				needRestart = true
			}
		}
	}
	...
	if len(oldEmail) > 0 {
		...
		if rterr != nil {
			...
			if err1 := rt.UpdateUser(context.Background(), oldInbound, oldEmail, clients[0]); err1 != nil {
				logger.Debug("UpdateUser on", rt.Name(), "failed (agent nodes don't expose this API):", err1)
				needRestart = true
			}
		}
	} else {
