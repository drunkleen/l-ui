...
import (
	...
	"github.com/drunkleen/l-ui/internal/util/netproxy"
	"github.com/drunkleen/l-ui/internal/util/random"
	"github.com/drunkleen/l-ui/internal/util/reflect_util"
...
func (s *SettingService) SetXrayOutboundTestUrl(url string) error {
	clean, err := util.SanitizeHTTPURL(url)
	if err != nil {
		return err
	}
...
func (s *SettingService) GetNodeCpuThreshold() (int, error) {
	return s.getInt("nodeCpuThreshold")
}

func (s *SettingService) GetNodeMemThreshold() (int, error) {
	return s.getInt("nodeMemThreshold")
}

func (s *SettingService) GetNodeDiskThreshold() (int, error) {
	return s.getInt("nodeDiskThreshold")
}
...
func (s *SettingService) SetCaCert(value string) error {
	return s.setString("caCert", value)
}

func (s *SettingService) GetCaCert() (string, error) {
	return s.getString("caCert")
}

func (s *SettingService) SetCaKey(value string) error {
	return s.setString("caKey", value)
}

func (s *SettingService) GetCaKey() (string, error) {
	return s.getString("caKey")
}
...
func (s *SettingService) GetIpLimitEnable() (bool, error) {
	if _, err := os.Stat(xray.GetConfigPath()); os.IsNotExist(err) {
		return false, nil
	}
...
func validateSettingsURLs(allSetting *entity.AllSetting) error {
	if allSetting.ExternalTrafficInformURI != "" {
		u, err := util.SanitizeHTTPURL(allSetting.ExternalTrafficInformURI)
		if err != nil {
	...
	if allSetting.TgBotAPIServer != "" {
		u, err := util.SanitizeHTTPURL(allSetting.TgBotAPIServer)
		if err != nil {
