...
func (s *ServerService) CleanupNodeInstall() error {
	_ = s.xrayService.StopXray()
	for _, args := range [][]string{{"stop", "caddy"}, {"disable", "caddy"}, {"stop", "l-ui"}, {"disable", "l-ui"}} {
...
func (s *ServerService) RotateBootstrapToken(token string) error {
...
func (s *ServerService) ReloadCaddy() error {
...
func parseUfwStatus(output string) UfwStatus {
	status := UfwStatus{Enabled: strings.Contains(strings.ToLower(output), "status: active")}
	for _, line := range strings.Split(output, "\n") {
		...
		numStr := strings.TrimSpace(strings.Trim(line[:closeIdx+1], "[]"))
		num, err := strconv.Atoi(numStr)
		if err != nil {
			continue
		}
		parts := strings.Fields(strings.TrimSpace(line[closeIdx+1:]))
		if len(parts) < 3 {
			continue
		}
		actionIdx := -1
		...
		if actionIdx <= 0 || actionIdx >= len(parts) {
			continue
		}
		to := strings.Join(parts[:actionIdx], " ")
		actionParts := parts[actionIdx:]
		if len(actionParts) > 1 {
			next := strings.ToUpper(actionParts[1])
			if next == "IN" || next == "OUT" {
				actionParts = actionParts[:2]
			} else {
				actionParts = actionParts[:1]
			}
		}
		from := ""
		if len(parts) > actionIdx+len(actionParts) {
			from = strings.Join(parts[actionIdx+len(actionParts):], " ")
		}
		status.Rules = append(status.Rules, UfwRule{Number: num, To: to, Action: strings.ToUpper(strings.Join(actionParts, " ")), From: from, Raw: line})
	}
	return status
}
...
func ufwRulePortMatches(rule UfwRule, port int, protocol string) bool {
	...
	return strings.HasPrefix(strings.ToLower(strings.TrimSpace(rule.To)), strings.ToLower(target))
}

func ufwActionMatches(rule UfwRule, action string) bool {
	return strings.HasPrefix(strings.ToUpper(strings.TrimSpace(rule.Action)), strings.ToUpper(strings.TrimSpace(action)))
}
...
func (s *ServerService) setUfwRule(action string, port int, protocol string) error {
	...
	for i := len(status.Rules) - 1; i >= 0; i-- {
		rule := status.Rules[i]
		if !ufwRulePortMatches(rule, port, proto) {
			continue
		}
		if ufwActionMatches(rule, action) {
			desiredPresent = true
			continue
		}
...
func (s *ServerService) AllowUfwPort(port int, protocol string) error {
	return s.setUfwRule("allow", port, protocol)
}

func (s *ServerService) DenyUfwPort(port int, protocol string) error {
	return s.setUfwRule("deny", port, protocol)
}

func (s *ServerService) ReinstallNodeBundle(file multipart.File) error {
	if file == nil {
		return fmt.Errorf("bundle file is required")
	}
	tmpFile, err := os.CreateTemp("", "l-ui-node-bundle-*.tar.gz")
	if err != nil {
		return err
	}
	...
	if err := os.MkdirAll("/var/log/l-ui", 0755); err != nil {
		return err
	}
	...
	for {
		...
		target := "/" + strings.TrimPrefix(filepath.Clean(hdr.Name), "/")
		if !strings.HasPrefix(target, "/usr/") && !strings.HasPrefix(target, "/etc/") {
			continue
		}
	...
	go func() {
		...
		_ = exec.Command("systemctl", "restart", "l-ui").Run()
	}()
	return nil
}
...
func (s *ServerService) GetLogs(count string, level string, syslog string) []string {
	...
	if syslog == "true" {
		...
		cmd := exec.Command("journalctl", "-u", "l-ui", "--no-pager", "-n", strconv.Itoa(countInt), "-p", level)
...
func (s *ServerService) importPostgresDB(file multipart.File) error {
	...
	tempFile, err := os.CreateTemp("", "l-ui-pg-restore-*.dump")
	if err != nil {
