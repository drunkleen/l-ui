...
// EnsureToken makes sure a named API token exists with the provided
// plaintext value. It is intended for node bootstrap, where the admin
// supplies a known secret and the agent stores only the SHA-256 hash.
func (s *ApiTokenService) EnsureToken(name, plaintext string) error {
	name = strings.TrimSpace(name)
	plaintext = strings.TrimSpace(plaintext)
	if name == "" || plaintext == "" {
		return common.NewError("token name and value are required")
	}
	db := database.GetDB()
	hashed := crypto.HashTokenSHA256(plaintext)
	var row model.ApiToken
	err := db.Where("name = ?", name).First(&row).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return db.Create(&model.ApiToken{Name: name, Token: hashed, Enabled: true}).Error
		}
		return err
	}
	return db.Model(&model.ApiToken{}).Where("id = ?", row.Id).Updates(map[string]any{
		"token":   hashed,
		"enabled": true,
	}).Error
}
