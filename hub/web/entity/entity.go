...
type Msg struct {
	...
	Code    string `json:"code,omitempty"` // Machine-readable error/code name
	Msg     string `json:"msg"`            // Response message text
	Obj     any    `json:"obj"`            // Optional data object
}

// AllSetting contains all configuration settings for the l-ui panel including web server, Telegram bot, and subscription settings.
type AllSetting struct {
	...
	TgCpu             int    `json:"tgCpu" form:"tgCpu" validate:"gte=0,lte=100"`                         // Hub CPU usage threshold for alerts (percent)
	NodeCpuThreshold  int    `json:"nodeCpuThreshold" form:"nodeCpuThreshold" validate:"gte=0,lte=100"`   // Node CPU threshold for alerts (percent)
	NodeMemThreshold  int    `json:"nodeMemThreshold" form:"nodeMemThreshold" validate:"gte=0,lte=100"`   // Node memory threshold for alerts (percent)
	NodeDiskThreshold int    `json:"nodeDiskThreshold" form:"nodeDiskThreshold" validate:"gte=0,lte=100"` // Node disk threshold for alerts (percent)
	NodeDownThreshold int    `json:"nodeDownThreshold" form:"nodeDownThreshold" validate:"gte=1,lte=360"` // Consecutive failed heartbeats before node-down alert
