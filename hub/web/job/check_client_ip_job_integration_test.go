...
import (
	...
	luilogger "github.com/drunkleen/l-ui/internal/logger"
)

// l-ui logger must be initialised once before any code path that can
...
func setupIntegrationDB(t *testing.T) {
	...
	t.Setenv("LUI_LOG_FOLDER", logDir)
...
func TestRun_DisabledFail2BanSkipsProbeAndBanLog(t *testing.T) {
	...
	t.Setenv("LUI_BIN_FOLDER", binDir)
...
func readIpLimitLogPath() string {
	folder := os.Getenv("LUI_LOG_FOLDER")
	if folder == "" {
